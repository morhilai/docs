...
{CODE-BLOCK: bash}
curl \
    'http://<server URL>/databases/<database name>/queries?queryHash=<20 digit hash> \
    -X POST \
    -d {
        "Query":<RQL query>,
        "QueryParameters":{<Parameters used in the query>}
    }
...
| - | - | - |
| queryHash | unique parameter for the query cache | Yes |

| Body Parameter | Description | Required |
| - | - | - |
| Query | A query written in RQL | Yes |
| QueryParameters | Parameters used in the query | No |
